...
QUERIES = [
    "I want to experiment with multi-agent problem solving where agents debate, specialize, and then converge. What patterns are worth trying first?",  # 0.071
    "What scaffolding do strong terminal-based coding agents use around the model itself?",                                                              # 0.154
    "What are the main failure modes of web agents in realistic browsing tasks, and how should I evaluate them?",                                       # 0.167
    "How would you build a GUI agent that can operate a clunky enterprise web app with inconsistent layouts?",                                          # 0.273
    "What should I measure to catch post-merge quality problems in agent-generated pull requests?",                                                     # 0.250
]
...
def fetch_rank_lists(retriever: Retriever, query: str, top_k: int) -> QueryData:
    ...
    We call the underlying Qdrant search twice with each vector type separately,
    then merge into a unified RankedChunk map.
    """
    qdata = QueryData(query=query)

    # Access the underlying Qdrant client via the retriever
    # We'll use retriever's existing search but exploit that named vectors
    # let us search dense-only or sparse-only by specifying the vector name.
    client = retriever._client  # qdrant_client.QdrantClient

    # Get the query embedding (dense)
    dense_vector = retriever._embed(query)  # returns List[float]

    # Get the sparse vector (SPLADE / MiniCOIL)
    sparse_vector = retriever._embed_sparse(query)  # returns SparseVector
