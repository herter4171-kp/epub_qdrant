...
from src.storage import Storage, _book_title_to_collection
...
def _process_book(
    ...
    # 4. Upsert to Qdrant
    count = storage.upsert_file(epub_path, all_chunks)
...
def ingest(directory: str, url: str, limit: int) -> None:
    ...
    for i, epub in enumerate(epubs, 1):
        ...
        try:
            count = _process_book(str(epub), embedder, storage)
            total += count
...
def search(collection: str, query: str, top_k: int) -> None:
    """Search a Qdrant collection for relevant passages."""
    storage = Storage()
    results = storage.search(collection, query, top_k=top_k)

    if not results:
        click.echo(f"No results found in collection '{collection}'.")
        return

    click.echo(f"\nResults for '{query}' in '{collection}':\n")
